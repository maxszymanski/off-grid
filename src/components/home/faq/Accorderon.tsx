...
function Accorderon({ onClick, isOpen, title, answer }: AccorderonProps) {
    return (
        <div className="bg-dark3 w-full overflow-hidden rounded-[14px] md:rounded-2xl">
            <h3 className="w-full">
                <button
                    className={`hover:bg-dark2/80 group relative flex w-full cursor-pointer items-center justify-between gap-5 rounded-[14px] p-[14px] text-left transition-colors duration-300 md:rounded-2xl md:px-8 md:py-[32.5px]`}
                    onClick={onClick}
                >
                    <span className="leading-[130%] md:text-2xl md:leading-[130%]">
                        {title}
                    </span>
                    <span className="bg-dark4 accorderon-icon group-hover:bg-dark pointer-events-none flex size-9 shrink-0 items-center justify-center rounded-lg transition-colors duration-300 md:absolute md:right-8 md:top-1/2 md:size-12 md:-translate-y-1/2 md:rounded-[10.5px]">
                        <HiPlus
